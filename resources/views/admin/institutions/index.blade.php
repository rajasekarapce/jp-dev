               ...
               <select name="state_id" required class="select2">
                <option value="">Please Select</option>
                  ...
                  <div class="form-check {{ $errors->has('state_id') ? 'is-invalid' : '' }}">
                     <option value="{{ $key }}" {{ old('qual_type') === $key ? 'selected' : '' }}>{{ $label }}</option>
                  </div>
                  @endforeach
               </select>
               {!! e_form_error('state_id', $errors) !!}
                ...
                <div class="form-group row">
                    <div class="col-sm-offset-4 col-sm-8">
                        <button type="submit" class="btn btn-primary">@lang('app.save_new_institution')</button>
                    </div>
                </div>
            ...
            <table class="table table-bordered">
                ...
                @foreach($institutions as $institution)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('edit_institutions', $institution->id) }}" class="btn btn-info"><i class="la la-pencil"></i> </a>
                            <a href="javascript:;" class="btn btn-danger category_delete" data-id="{{ $institution->id }}"><i class="la la-trash"></i> </a>
                        </td>
                    </tr>
                @endforeach
            </table>
