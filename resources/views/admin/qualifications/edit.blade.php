        ...
        <div class="col-md-10">
            ...
            <form method="post" action="">
                ...
                <div class="form-group row {{ $errors->has('course')? 'has-error':'' }}">
                    ...
                    <div class="col-sm-8">
                        <input type="text" class="form-control {{e_form_invalid_class('course', $errors)}}" id="course" value="{!! $qualification->course !!}" name="course" placeholder="@lang('app.course')">
                ...
                <div class="form-group row {{ $errors->has('qual_type')? 'has-error':'' }}">
                    ...
                    <div class="form-check {{ $errors->has('qual_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="qual_type" value="{{ $key }}" {{ old('qual_type', $qualification->qual_type) === $key ? 'checked' : '' }} required>
                ...
                <div class="form-group row {{ $errors->has('status')? 'has-error':'' }}">
                    ...
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $qualification->status) === $key ? 'checked' : '' }} required>
                        <label class="form-check-label" for="status_{{ $key }}">{{ $label }}</label>
                    </div>
                @endforeach
                 ...
                 <div class="form-group row {{ $errors->has('popular')? 'has-error':'' }}">
                    ...
                    <div class="form-check {{ $errors->has('popular') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="popular_{{ $key }}" name="popular" value="{{ $key }}" {{ old('popular', $qualification->popular) === $key ? 'checked' : '' }} required>
                        <label class="form-check-label" for="status_{{ $key }}">{{ $label }}</label>
                    </div>
                @endforeach

                        {!! e_form_error('popular', $errors) !!}
                    </div>
                </div>
                ...
                <div class="form-group row">
                    <div class="col-sm-offset-4 col-sm-8">
                        <button type="submit" class="btn btn-primary">@lang('app.update_qualification')</button>
                    </div>
                </div>
            </form>
        </div>
